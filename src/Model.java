...
public class Model
{
  ...
  int NLead;       // maximum lead in model
    ...
    //  Equation[] UnsortedEquations = new Equation[Aim.Max_Array_Size]; // array of 
  ...
  Variable[] Variables = new Variable[Aim.Max_Array_Size];  // Variable objects
    //  String[] EndogenousVariables = new String[Aim.Max_Array_Size]; // endog var names
  ...
  public int ErrorCheck() { 
	...
	int errors = 0;
	int[] EquationsNamedAfterVariable = new int[Aim.Max_Array_Size];
	int vindex;
	int eindex;
	...
	for (vindex = 0; vindex < NVars; vindex++) // check the results
		...
		if ((Variables[vindex].Type == Aim.Endogenous) &&
		    (EquationsNamedAfterVariable[vindex] == 0)) {
		...
		else if ((Variables[vindex].Type == Aim.Endogenous) &&
 		...
 		else if ((Variables[vindex].Type == Aim.Exogenous) &&
		    (EquationsNamedAfterVariable[vindex] > 0)) {
  ...
  public void BuildExogenousEquations() {
      ...
      onev.setType(Aim.Exogenous);
      onev.setDataType(Aim.Dtrm);
      AddVariable(onev);

      for (i = 0; i < NVars; i++) {
	  if (Variables[i].Type == Aim.Exogenous) {
	      if (Variables[i].DataType == Aim.Innov) {
		  ...
		  if (index < 0) {
		      ...
		      e = new Equation(Variables[i].Name, vn1, pn, Aim.Stoch);
		      AddEquation(e);
		  } // end of "if (index < 0)" block
	      } // end of "if (Variables[i].DataType == Aim.Innov)" block
	      else if (Variables[i].DataType == Aim.Dtrm) {
		  index = FindEquationIndex(Variables[i].Name);
		  if (index < 0) {
		      vn1 = new VariableNode(Variables[i].Name, 0, 0);
		      vn2 = new VariableNode(Variables[i].Name, -1, 0);
		      e = new Equation(Variables[i].Name, vn1, vn2, Aim.Imposed);
		      AddEquation(e);
		  } // end of "if (index < 0)" block
	      } // end of "else if (Variables[i].DataType == Aim.Dtrm)" block
	  } // end of "if (Variables[i].Type == Aim.Exogenous)" block
      } // end of for loop
  }
	...
	for (i = 0; i < NVars; i++) {
		...
		System.out.print(" ");
	    switch (Variables[i].Type) {
	    case Aim.Endogenous: System.out.print("Endogenous  "); break;
	    case Aim.Exogenous: System.out.print("Exogenous   "); break;
	    }
	    switch (Variables[i].DataType) {
	    case Aim.Data: System.out.print("Data   "); break;
	    case Aim.Notd: System.out.print("Notd   "); break;
	    case Aim.Dtrm: System.out.print("Dtrm   "); break;
	    case Aim.Innov: System.out.print("Innov  "); break;
	    }
	    switch (Equations[FindEquationIndex(Variables[i].Name)].getEqType()) {
	    case Aim.Stoch: System.out.print("Stochastic"); break;
	    case Aim.Imposed: System.out.print("Imposed   "); break;
	    }
	    System.out.println("");
	}
    }
